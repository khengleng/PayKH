...
export class AiService {
  ...
  private label(text: string | null, fallback: string): { text: string; source: 'ai' | 'computed' } {
    return text ? { text: text.trim(), source: 'ai' } : { text: fallback, source: 'computed' };
  }
  ...
  async marketingCopy(user: AuthUser, storeId: string, dto: MarketingCopyDto) {
    ...
    const ai = await this.llm.complete(
  ...
  async suggestCampaign(user: AuthUser, storeId: string) {
    ...
    return { ...this.label(ai, fallback), context: ctx };
  }
  ...
  async analyticsSummary(user: AuthUser, storeId: string) {
    ...
    return { ...this.label(ai, fallback), metrics: ctx };
  }
  ...
  async fraudInsights(user: AuthUser, storeId: string) {
    ...
    const ai = await this.llm.complete(
      ...
      `Open cases: ${cases.length}. Reason frequency: ${JSON.stringify(reasonCounts)}. Top scores: ${cases.slice(0, 5).map((c) => c.score).join(', ')}.`,
      300,
    );
    ...
    return { ...this.label(ai, fallback), open_cases: cases.length, reason_counts: reasonCounts };
  }
  ...
  async assistant(user: AuthUser, storeId: string, dto: AssistantDto) {
    ...
    return { ...this.label(ai, fallback), question: dto.question };
  }
}
