...
function MiniApp() {
  ...
  if (error) return <Screen><Card><p className="text-center text-sm text-red-600">{error}</p></Card></Screen>;
  if (!session || !me) return <Screen><p className="mt-20 text-center text-slate-400">Loading your rewards…</p></Screen>;
  ...
  return (
    <Screen>
      ...
      <div className="mt-6 flex gap-2">
        <button onClick={() => setView('qr')} className="flex-1 rounded-xl bg-indigo-600 py-2.5 text-sm font-semibold text-white">My member QR</button>
        <button onClick={() => setView('history')} className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-medium text-slate-600">History</button>
      </div>
      <div className="mt-5 space-y-2">
        ...
        {me.merchants.map((m) => (
          <button key={m.customer_id} onClick={() => setDetail(m)} className="flex w-full items-center gap-3 rounded-xl border border-slate-100 bg-white p-3 text-left shadow-sm active:bg-slate-50">
      ...
      <div className="mt-2 space-y-2">
        ...
        {(w?.rewards ?? []).map((r: any) => (
          <div key={r.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-white p-3">
            ...
            <button disabled={!r.affordable || !r.in_stock || busy === r.id} onClick={() => redeem(r.id)} className="rounded-lg bg-indigo-600 px-3 py-1.5 text-sm font-semibold text-white disabled:opacity-40">
...
function Screen({ children }: { children: React.ReactNode }) {
  return <main className="mx-auto min-h-screen max-w-md bg-slate-50 px-4 pb-10">{children}</main>;
}
