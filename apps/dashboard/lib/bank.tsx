...
/** Payer-facing "who you're paying" data from the merchant's imported KHQR. */
...
/**
 * Bank brand mark. We don't ship copyrighted logo images, so each bank shows as
 * a wordmark badge in its brand colour — recognisable and CSP-safe. Swap for a
 * real logo <img> later if the assets are available.
 */
...
export function BankMark({ code, name, size = 36 }: { code: string | null; name: string | null; size?: number }) {
...
/** "Paying to" card: bank mark + owner + bank + account. */
export function PayeeCard({ payee, label = 'Paying to', className = '' }: { payee: Payee; label?: string; className?: string }) {
  return (
    <div className={`flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3 text-left ${className}`}>
